...
class PersonHandler:
    """Handler for moving persons on the ground."""
    ...
    def __init__(
        ...
        # Persons are always at ground level (z=0)
        self.targets = np.zeros(shape=(self.num_persons, 3))

    def reset(
        self,
        np_random: np.random.Generator = None
    ):
        """Resets persons to random ground positions."""
        # store the client
        #self.p = p
        ...
        # Random spawn within the dome on the X-Y plane
        # It doesn't start in groups
    ...
    def get_relative_deltas(self, drone_pos: np.ndarray):
        """Returns the linear distance vector from drone to each person."""
        # We only care about X and Y deltas since drone altitude is fixed
        return self.targets - drone_pos
